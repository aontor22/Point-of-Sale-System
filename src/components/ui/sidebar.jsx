  ...
  return (
    <>
      <button
        ...
        className="absolute top-5 z-[60] grid h-5 w-5 place-items-center rounded-full bg-[#0e1074] text-white shadow-lg ring-1 ring-black/5 hover:bg-[#0e3070] transition"
      ...
      <aside className={`shrink-0 border-r ${expanded ? "w-[220px]" : "w-[70px]"} h-auto min-h-screen flex flex-col`}>
...
export function SidebarItem({
  ...
  const Label = (
    <>
      {sizedIcon}
      <span className={`overflow-hidden transition-all ${expanded ? "flex-1 ms-3 text-left" : "w-0"}`}>
        {text}
      </span>
      {isParent && expanded && (
  ...
  if (isParent) {
    return (
      <li>
        <button type="button" onClick={onClick} className={`w-full ${base}`} aria-expanded={open}>
          {Label}
        </button>
      </li>
    );
    ...
    <li>
      {to ? (
        <NavLink
          to={to}
          className={({ isActive }) =>
            `w-full block ${base} ${isActive ? "bg-indigo-100 text-indigo-800" : ""}`
          }
        >
          {Label}
        </NavLink>
